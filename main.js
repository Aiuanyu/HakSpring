...
function createMobileLookupButton(popupEl, contentEl, backdropEl) {
  ...
  mobileLookupButton.addEventListener('click', () => {
    ...
    if (selection && selection.toString().trim().length > 0 && lastSelectionRectForMobile) {
      const selectedText = selection.toString().trim();
      // The showPronunciationPopup function now handles data fetching asynchronously.
      // We no longer need to call findPronunciationsInAllData here. We pass `null` for the readings.
      showPronunciationPopup(selectedText, null, lastSelectionRectForMobile, null);
      hideMobileLookupButton();
    } else {
      hideMobileLookupButton();
    }
