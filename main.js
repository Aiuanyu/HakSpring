...
function handleTextSelectionInSentence(event, popupEl, contentEl, backdropEl, generatedArea) {
  ...
  setTimeout(() => {
    ...
    if (isDebugMode) console.log('[DEBUG] Common Ancestor Container:', commonAncestor);

    const parentElement = commonAncestor.nodeType === Node.TEXT_NODE ? commonAncestor.parentElement : commonAncestor;
    if (isDebugMode) console.log('[DEBUG] Starting search for sentence span from:', parentElement);

    const sentenceSpan = parentElement.closest('span.sentence');
    if (isDebugMode) console.log('[DEBUG] Result of .closest("span.sentence"):', sentenceSpan);
